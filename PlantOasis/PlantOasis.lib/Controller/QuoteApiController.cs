    ...
    public class QuoteApiController : _BaseApiController
    {
        ...
        public Api_AddToBasketInfo AddProductToQuote(string id)
        {
            try
            {
                ...
                Product product = InsertProduct2QuoteModel.AddProductToQuote(quote.pk, new Guid(pkProduct), cnt);
        ...
        public Api_BasketInfo RemoveDiscountCouponToQuote(string id)
        {
            try
            {
                ...
                QuoteRepository quoteRep = new QuoteRepository();
                Quote quote = quoteRep.GetForSession(quoteId);
                new Product2QuoteRepository().DeleteNonProductItem(quote.pk, ConfigurationUtil.PlantOasis_Quote_DiscountItemCode);
            ...
            try
            {
                ...
                if (quoteKey != null && quoteKey != Guid.Empty)
                {
                    ...
                    if (quoteItems.Count > 0)
                    {
                        ...
                        List<Product> productList = prodRep.GetPageForQuote(1, _PagingModel.AllItemsPerPage, quote.pk).Items;
        ...
        decimal GetQuotePriceForFreeTransport(List<Product2Quote> quoteItems)
        {
            ...
            foreach (Product2Quote item in quoteItems)
            {
                if (item.ItemCode != ConfigurationUtil.PlantOasis_Quote_PaymentItemCode && item.ItemCode != ConfigurationUtil.PlantOasis_Quote_TransportItemCode)
                {
