...
namespace PlantOasis.lib.Controller
{
    ...
    public class QuoteController : _BaseController
    {
        ...
        {
            QuoteModel.RecalcQuoteDiscountForSession(this.CurrentSessionId, CustomerCache.CurrentMemberId);
        ...
        public ActionResult BasketDeliveryData()
        {
            ...
            if ((model.CurrentQuote.User.IsNew || model.CurrentQuote.User.IsEmpty) && CustomerCache.IsCustomerAuthenticated)
            {
                EshopCustomer customer = CustomerCache.GetCurrentCustomer();
                if (customer != null)
                {
        ...
        public ActionResult SubmitBasketDeliveryData(BasketModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (transportType != null)
                {
                    ...
                    Product2Quote transportItem = rep.GetForItemCode(model.CurrentQuote.pk, ConfigurationUtil.PlantOasis_Quote_TransportItemCode);
                ...
                {
                    Product2QuoteRepository rep = new Product2QuoteRepository();
                    Product2Quote paymentItem = rep.GetForItemCode(model.CurrentQuote.pk, ConfigurationUtil.PlantOasis_Quote_PaymentItemCode);
                    if (paymentItem == null)
                    {
            ...
            if (ModelState.IsValid)
            {
                if (CustomerCache.IsCustomerAuthenticated)
                {
                    EshopCustomer customer = CustomerCache.GetCurrentCustomer();
                    if (customer != null)
                    {
            ...
            if (ModelState.IsValid)
            {
                ...
                //else
                {
                    return this.RedirectToUmbracoPage(ConfigurationUtil.PlantOasis_Basket_ReviewAndSendPageId);
                }
            }
        ...
        public ActionResult QuoteSendStateNotification(string id)
        {
            ...
            model.CurrentQuote = GetQuoteWithProducts(new Guid(id));
            model.CurrentQuote.User = GetQuoteUser(model.CurrentQuote);
            QuoteNotifier.SendQuoteStateNotification(model.CurrentQuote.pk, this.DefaultImgPath, false, notifyPlantOasis: false);
