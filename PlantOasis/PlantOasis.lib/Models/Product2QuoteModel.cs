...
using static PlantOasis.lib.Models.ProductPagingListModel;
...
namespace PlantOasis.lib.Models
{
    ...
    public class InsertProduct2QuoteModel
    {
        ...
        public static Product AddProductToQuote(Guid pkQuote, Guid pkProduct, decimal cnt)
        {
            // Get product
            ProductRepository prodRep = new ProductRepository();
            Product product = prodRep.Get(pkProduct);
            ...
            if (dataRec == null)
            {
                ...
                ProductPriceInfo productPrice = ProductPriceCache.GetProductPrice(dataRec.PkProduct);
