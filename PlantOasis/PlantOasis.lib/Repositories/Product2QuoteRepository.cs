...
namespace PlantOasis.lib.Repositories
{
    public class Product2QuoteRepository : _BaseRepository
    {
        ...
        public Page<Product2QuotePcs> GetProduct2QuotePcsPage(long page, long itemsPerPage)
        {
            ...
            //var sql = new Sql("select pkProduct, sum(itemPcs) as pcs from poProduct2Quote where pkQuote in (SELECT pk from poQuote where dateFinished is not null and sessionId is null) and pkProduct is not null and pkProduct != '00000000-0000-0000-0000-000000000000' and pkProduct in (SELECT pk from poProduct where productIsVisible=1) group by pkProduct order by pcs desc");
            var sql = new Sql("select pkProduct, count(itemPcs) as pcs from poProduct2Quote where pkQuote in (SELECT pk from poQuote where dateFinished is not null and sessionId is null) and pkProduct is not null and pkProduct != '00000000-0000-0000-0000-000000000000' and pkProduct in (SELECT pk from poProduct where productIsVisible=1) group by pkProduct order by pcs desc");
        ...
        public void DeleteNonProductItems(Guid keyQuote)
        {
            if (keyQuote != null && keyQuote != Guid.Empty)
            {
                ...
                sql.Where(GetNotItemCodeWhereClause(), new { NotItemCode = ConfigurationUtil.Plantoasis_Quote_DiscountItemCode });
    ...
    {
        public const string DbTableName = "poProduct2Quote";
